...
export const constantRoutes = [
  ...
  {
    path: '/dashboard',
    component: Layout, // 使用公共布局 Layout
    redirect: '/dashboard/index', // 访问 /dashboard 时重定向到子路由
    children: [
      {
        path: 'index',
        // 因为要求“和首页一样”，这里直接复用首页的组件。
        // 如果后续开发了独立的仪表盘页面，请将路径改为: import('@/views/dashboard/index')
        component: () => import('@/views/dashboard/index'),
        name: 'Dashboard', // 路由名称，建议大写开头
        meta: {
          title: '仪表盘',  // 侧边栏菜单显示的文字
          icon: 'chart',   // 菜单图标 (确保 src/assets/icons/svg 下有 chart.svg，没有则改为 dashboard)
          affix: false     // 是否固定在 tagsView 栏上 (false 表示不固定)
        }
      }
    ]
  },
...
export const dynamicRoutes = [
  ...
  },
  {
    ...
    component: Layout,
    hidden: true,
    permissions: ['monitor:threadPool:list'],
    children: [
      {
        path: 'index',
        component: () => import('@/views/monitor/threadPool/index'),
        name: 'ThreadPool',
        meta: {title: '线程池监控', activeMenu: '/monitor/threadPool'}
      }
    ]
  },
  {
