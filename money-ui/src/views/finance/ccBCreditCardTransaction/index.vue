<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['finance:ccb_credit_card_transaction:add']"
        >新增
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['finance:ccb_credit_card_transaction:edit']"
        >修改
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['finance:ccb_credit_card_transaction:remove']"
        >删除
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['finance:ccb_credit_card_transaction:export']"
        >导出
        </el-button>
        <el-button
          type="success"
          ...
          size="mini"
          @click="handleQuerySync"
          v-hasPermi="['finance:ccb_credit_card_transaction:queryCCBCreditCardTransaction']"
        >获取
        </el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <el-table v-loading="loading" :data="ccBCreditCardTransactionList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="交易入账日期" align="center" prop="postDate" width="180">
        <template slot-scope="scope">
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="交易发生日期，格式 yyyy-MM-dd" prop="tradeDate">
          <el-date-picker clearable
                          ...
                          type="date"
                          value-format="yyyy-MM-dd"
                          placeholder="请选择交易发生日期，格式 yyyy-MM-dd">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="交易入账日期，格式 yyyy-MM-dd" prop="postDate">
          <el-date-picker clearable
                          ...
                          type="date"
                          value-format="yyyy-MM-dd"
                          placeholder="请选择交易入账日期，格式 yyyy-MM-dd">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="信用卡尾号后四位" prop="cardLast4">
          <el-input v-model="form.cardLast4" placeholder="请输入信用卡尾号后四位"/>
        </el-form-item>
        <el-form-item label="交易描述信息" prop="description">
          <el-input v-model="form.description" type="textarea" placeholder="请输入内容"/>
        </el-form-item>
        <el-form-item label="交易原始金额，格式 币种/金额" prop="transAmount">
          <el-input v-model="form.transAmount" placeholder="请输入交易原始金额，格式 币种/金额"/>
        </el-form-item>
        <el-form-item label="结算金额，格式 币种/金额" prop="settleAmount">
          <el-input v-model="form.settleAmount" placeholder="请输入结算金额，格式 币种/金额"/>
        </el-form-item>
...
<script>
import {
  listCcBCreditCardTransaction,
  getCcBCreditCardTransaction,
  delCcBCreditCardTransaction,
  addCcBCreditCardTransaction,
  updateCcBCreditCardTransaction,
  queryCCBCreditCardTransaction,
  importCCBCreditCardTransaction
} from "@/api/finance/ccBCreditCardTransaction";
import {listUser} from "@/api/stock/dropdown_component";  // 获取用户列表API

export default {
  name: "CcBCreditCardTransaction",
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      rules: {
        tradeDate: [
          {required: true, message: "交易发生日期，格式 yyyy-MM-dd不能为空", trigger: "blur"}
        ],
        postDate: [
          {required: true, message: "交易入账日期，格式 yyyy-MM-dd不能为空", trigger: "blur"}
        ],
        cardLast4: [
          {required: true, message: "信用卡尾号后四位不能为空", trigger: "blur"}
        ],
        description: [
          {required: true, message: "交易描述信息不能为空", trigger: "blur"}
        ],
        transAmount: [
          {required: true, message: "交易原始金额，格式 币种/金额不能为空", trigger: "blur"}
        ],
        settleAmount: [
          {required: true, message: "结算金额，格式 币种/金额不能为空", trigger: "blur"}
        ],
      }
    };
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCcBCreditCardTransaction(this.queryParams).then(response => {
        this.ccBCreditCardTransactionList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加建行信用卡交易记录";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getCcBCreditCardTransaction(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改建行信用卡交易记录";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCcBCreditCardTransaction(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCcBCreditCardTransaction(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除建行信用卡交易记录编号为"' + ids + '"的数据项？').then(function () {
        return delCcBCreditCardTransaction(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('finance/ccb_credit_card_transaction/export', {
        ...this.queryParams
      }, `ccBCreditCardTransaction_${new Date().getTime()}.xlsx`)
    },
    /** 同步按钮操作 */
    handleQuerySync() {
      this.$modal.confirm('确认同步数据？')
        .then(() => {
          return queryCCBCreditCardTransaction();
        })
        .then(() => {
          this.getList();
    ...
    confirmImport() {
      ...
      importCCBCreditCardTransaction(this.selectedFile)
        .then(response => {
