<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="买卖方向 (1:买入, 2:卖出, 4:中性)" prop="sideCode">
        <el-input
          v-model="queryParams.sideCode"
          placeholder="请输入买卖方向 (1:买入, 2:卖出, 4:中性)"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="该时刻包含的成交笔数" prop="tickCount">
        <el-input
          v-model="queryParams.tickCount"
          placeholder="请输入该时刻包含的成交笔数"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="每笔平均成交量" prop="avgVol">
        <el-input
          v-model="queryParams.avgVol"
          placeholder="请输入每笔平均成交量"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="是否判定为大资金入场 (0:否, 1:是)" prop="isBigMoney">
        <el-input
          v-model="queryParams.isBigMoney"
          placeholder="请输入是否判定为大资金入场 (0:否, 1:是)"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['stock:stockTick:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['stock:stockTick:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['stock:stockTick:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['stock:stockTick:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="stockTickList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="买卖方向 (1:买入, 2:卖出, 4:中性)" align="center" prop="sideCode" />
      <el-table-column label="该时刻包含的成交笔数" align="center" prop="tickCount" />
      <el-table-column label="每笔平均成交量" align="center" prop="avgVol" />
      <el-table-column label="是否判定为大资金入场 (0:否, 1:是)" align="center" prop="isBigMoney" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="买卖方向 (1:买入, 2:卖出, 4:中性)" prop="sideCode">
          <el-input v-model="form.sideCode" placeholder="请输入买卖方向 (1:买入, 2:卖出, 4:中性)" />
        </el-form-item>
        <el-form-item label="该时刻包含的成交笔数" prop="tickCount">
          <el-input v-model="form.tickCount" placeholder="请输入该时刻包含的成交笔数" />
        </el-form-item>
        <el-form-item label="每笔平均成交量" prop="avgVol">
          <el-input v-model="form.avgVol" placeholder="请输入每笔平均成交量" />
        </el-form-item>
        <el-form-item label="是否判定为大资金入场 (0:否, 1:是)" prop="isBigMoney">
          <el-input v-model="form.isBigMoney" placeholder="请输入是否判定为大资金入场 (0:否, 1:是)" />
        </el-form-item>
      </el-form>
...
<script>
import { listStockTick, getStockTick, delStockTick, addStockTick, updateStockTick } from "@/api/stock/stockTick"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        sideCode: [
          { required: true, message: "买卖方向 (1:买入, 2:卖出, 4:中性)不能为空", trigger: "blur" }
        ],
      }
    }
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listStockTick(this.queryParams).then(response => {
        this.stockTickList = response.rows
    ...
    handleAdd() {
      this.reset()
      this.open = true
      this.title = "添加股票逐笔成交明细"
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset()
      const id = row.id || this.ids
      getStockTick(id).then(response => {
        this.form = response.data
        this.open = true
        this.title = "修改股票逐笔成交明细"
      })
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateStockTick(this.form).then(response => {
              this.$modal.msgSuccess("修改成功")
              this.open = false
              this.getList()
            })
          } else {
            addStockTick(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除股票逐笔成交明细编号为"' + ids + '"的数据项？').then(function() {
        return delStockTick(ids)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('stock/stockTick/export', {
        ...this.queryParams
      }, `stockTick_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
