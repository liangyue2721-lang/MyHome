<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['stock:stock_info_dongfang_his:remove']"
        >删除
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['stock:stock_info_dongfang_his:export']"
        >导出
        </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="stock_info_dongfang_hisList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center"/>
      <el-table-column label="ID" align="center" prop="id"/>
      <!--      <el-table-column label="股票类型" align="center" prop="type" />-->
      <el-table-column label="保存时间" align="center" prop="inDate" width="180">
...
<!--      <el-table-column label="涨跌幅" align="center" prop="netChangePercentage"/>-->
      ...
      <el-table-column label="股票的交易量" align="center" prop="volume"/>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['stock:stock_info_dongfang_his:edit']"
          >修改
          </el-button>
        </template>
      </el-table-column>
...
<script>
import {
  listStock_info_dongfang_his,
  getStock_info_dongfang_his,
  delStock_info_dongfang_his,
  addStock_info_dongfang_his,
  updateStock_info_dongfang_his
} from "@/api/stock/stock_info_dongfang_his"

export default {
  name: "Stock_info_dongfang_his",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listStock_info_dongfang_his(this.queryParams).then(response => {
        this.stock_info_dongfang_hisList = response.rows
    ...
    handleUpdate(row) {
      ...
      getStock_info_dongfang_his(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateStock_info_dongfang_his(this.form).then(response => {
              this.$modal.msgSuccess("修改成功")
              this.open = false
              this.getList()
            })
          } else {
            addStock_info_dongfang_his(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除东方财富历史编号为"' + ids + '"的数据项？').then(function () {
        return delStock_info_dongfang_his(ids)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('stock/stock_info_dongfang_his/export', {
        ...this.queryParams
      }, `stock_info_dongfang_his_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
