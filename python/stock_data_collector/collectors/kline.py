...
from utils import (
    _log, redis_key, _fetch_json, _secid,
...
def get_kline_stocks():
    ...
    # 1. 从Redis获取涨停池股票
    if REDIS_AVAILABLE:
        zt_keys = rds.lrange(redis_key(MODULE_PREFIX, "limit_up_stocks"), 0, -1)
        for k in zt_keys:
    ...
    # 2. 从Redis获取竞价高开股
    if REDIS_AVAILABLE:
        hot_keys = rds.lrange(redis_key(MODULE_PREFIX, "auction_hot"), 0, 49)
        for k in hot_keys:
            if isinstance(k, bytes):
                k = k.decode()
            parts = k.split('.')
