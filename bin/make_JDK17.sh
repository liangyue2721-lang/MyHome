...
if [ ! -x "$JAVA_CMD" ]; then
    echo -e "\033[0;31m[错误]\033[0m 未找到 JDK17，请先执行："
    echo -e "  \033[0;33mdnf -y install java-17-openjdk-devel\033[0m"
    exit 1
fi
...
-Xlog:gc*,gc+heap=info,gc+age=trace:file=$GC_LOG:time,uptime,level,tags
"
...
start() {
    ...
    log "\033[0;32m[启动中]\033[0m 使用 JDK17 启动 $APP_NAME ..."
