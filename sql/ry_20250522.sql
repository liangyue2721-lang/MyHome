...
CREATE TABLE server_info (
                             ...
                             provider VARCHAR(100) DEFAULT NULL COMMENT '服务提供商（如阿里云、腾讯云、AWS）',
                             os_type VARCHAR(50) DEFAULT NULL COMMENT '操作系统类型（如CentOS 7、Ubuntu 20.04）',
                             purchase_date DATE DEFAULT NULL COMMENT '购买日期',
                             expire_date DATE NOT NULL COMMENT '到期日期',
                             remind_days INT DEFAULT 7 COMMENT '到期前提醒天数（单位：天）',
                             price DECIMAL(10,2) DEFAULT NULL COMMENT '购买或续费价格（元）',
                             status VARCHAR(20) DEFAULT 'active' COMMENT '状态（active=使用中, expiring=即将到期, expired=已过期, stopped=停用）',
                             admin_user VARCHAR(50) DEFAULT NULL COMMENT '负责人姓名',
                             contact_email VARCHAR(100) DEFAULT NULL COMMENT '负责人邮箱',
                             notify_sent TINYINT(1) DEFAULT 0 COMMENT '是否已发送到期提醒（0=否,1=是）',
                             remark TEXT COMMENT '备注信息',
                             create_time DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                             update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    -- ✅ 联合索引与单列索引
                             INDEX idx_expire_status (status, expire_date),          -- 按状态和到期时间查询
                             INDEX idx_expire_remind (notify_sent, expire_date),     -- 查找未提醒且即将到期的
                             INDEX idx_provider_admin (provider, admin_user),        -- 同一厂商或负责人下的服务器
                             INDEX idx_ip_address (ip_address)                    -- 按IP快速定位服务器
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='服务器有效期管理表（MySQL5.7兼容版）';
